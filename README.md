...
It uses a fuzzy, per word search powered by fuse.js, so it will find slightly misspelled words. In some cases that could lead some false matches. When searching for multiple word phrases, it will return entries that have matches for ALL searched words independent of there order. "Combat Jump" will find "Jumping during combat" but not "Combat" or "Jumping" 
